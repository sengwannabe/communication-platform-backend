...
describe.each(tests)('Expected Cases', ({ setupScript, checkScript }) => {
  test('Local owner pins', () => {
    const chatId = setupScript();
    expect(messagePinV1(users[1].token, message.messageId)).toStrictEqual({});
  ...
  test('Global owner pins', () => {
    const chatId = setupScript();
    expect(messagePinV1(users[0].token, message.messageId)).toStrictEqual({});
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
...
describe.each(tests)('Error Cases', ({ setupScript, checkScript, leaveScript }) => {
  test('invalid token', () => {
    const chatId = setupScript();
    expect(() => messagePinV1('', message.messageId)).toThrow(Error(FORBIDDEN));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('messageId does not refer to a valid message', () => {
    setupScript();
    expect(() => messagePinV1(users[1].token, message.messageId + 1)).toThrow(Error(BAD_REQUEST));
  });

  test('messageId in a chat the user is not a part of', () => {
    const chatId = setupScript();
    leaveScript(users[0].token, chatId);
    expect(() => messagePinV1(users[0].token, message.messageId)).toThrow(Error(BAD_REQUEST));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('message in a chat already contains a pin from the user', () => {
    const chatId = setupScript();
    expect(() => messagePinV1(users[1].token, message.messageId)).not.toThrow();
    expect(() => messagePinV1(users[1].token, message.messageId)).toThrow(Error(BAD_REQUEST));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('messageId in a chat the user does not have permissions in', () => {
    const chatId = setupScript();
    expect(() => messagePinV1(users[2].token, message.messageId)).toThrow(Error(FORBIDDEN));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
