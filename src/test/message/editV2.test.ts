import {
  clearV1, authRegisterV3, messageEditV2,
...
import {
  lorem1000char, lorem1001char, generateThreeUsers,
...
describe.each(tests)('Expected Cases - $source', ({ setupScript, sendScript, checkScript }) => {
  test('One $source, One Message', () => {
    ...
    expect(messageEditV2(users[1].token, message.messageId, messagesData[1])).toStrictEqual({});
  ...
  test('1000 character long message in a $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    const lorem = lorem1000char();
    expect(messageEditV2(users[1].token, message.messageId, lorem)).toStrictEqual({});
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
...
describe.each(tests)('Owner Cases - $source', ({ setupScript, sendScript, checkScript }) => {
  ...
  test('User is global owner in $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    expect(messageEditV2(users[0].token, message.messageId, messagesData[1])).toStrictEqual({});
...
describe.each(tests)('Deleting messages - $source', ({ setupScript, sendScript, checkScript }) => {
  test('Single deleted message in $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    expect(messageEditV2(users[1].token, message.messageId, '')).toStrictEqual({});
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [],
      start: 0,
      end: -1,
  ...
  test('Multiple messages, first message deleted in $source', () => {
    ...
    expect(messageEditV2(users[1].token, message1.messageId, '')).toStrictEqual({});
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
  ...
  test('Multiple messages, last message deleted in $source', () => {
    ...
    const message1 = sendScript(users[1].token, chatId, messagesData[0]);
    const message2 = sendScript(users[1].token, chatId, messagesData[1]);
    expect(messageEditV2(users[1].token, message2.messageId, '')).toStrictEqual({});
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
...
describe.each(tests)('Error Cases - $source', ({ setupScript, sendScript, checkScript }) => {
  test('length of message is over 1000 characters in $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    expect(() => messageEditV2(users[1].token, message.messageId, lorem1001char())).toThrow(Error(BAD_REQUEST));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('messageId does not refer to a valid message within a $source that the authorised user has joined', () => {
    const chatId = setupScript();
    expect(() => messageEditV2(users[1].token, 0, messagesData[1])).toThrow(Error(BAD_REQUEST));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [],
      start: 0,
      end: -1,
  ...
  test('user is not a member of the $source', () => {
    ...
    expect(() => messageEditV2(nonMember.token, message.messageId, messagesData[1])).toThrow(Error(BAD_REQUEST));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('authUser is not the sender and does not have owner permissions in the $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    expect(() => messageEditV2(users[2].token, message.messageId, messagesData[1])).toThrow(Error(FORBIDDEN));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
  ...
  test('Invalid Token in $source', () => {
    const chatId = setupScript();
    const message = sendScript(users[1].token, chatId, messagesData[0]);
    expect(() => messageEditV2('', message.messageId, messagesData[1])).toThrow(Error(FORBIDDEN));
    expect(checkScript(users[1].token, chatId, 0)).toStrictEqual({
      messages: [
        {
          messageId: message.messageId,
